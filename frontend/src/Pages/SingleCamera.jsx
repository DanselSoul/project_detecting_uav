...
import { useWebSocket, WebSocketProvider } from "../WebSocketProvider/WebSocketProvider";

function SingleCameraContent() {
  const { id } = useParams();
  const navigate = useNavigate();
  const { message, alertCamera } = useWebSocket();

  const [isDroneConfirmed, setIsDroneConfirmed] = useState(null);
  const [showAlert, setShowAlert] = useState(false);
  const [consecutiveFrames, setConsecutiveFrames] = useState(0);
  const [isObjectDetected, setIsObjectDetected] = useState(false);

  // Увеличиваем счётчик сообщений, если уведомление касается текущей камеры
  useEffect(() => {
  ...
  // Если набежало достаточное число сообщений, показываем окно подтверждения
  useEffect(() => {
  ...
  return (
    <div className="p-4">
      <button onClick={() => navigate(-1)} className="mb-4 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">
        Назад
      </button>
      ...
      {showAlert && isDroneConfirmed === null && (
        <div className="alert-box bg-red-500 text-white p-4 rounded mt-4">
          <p>{message}</p>
          <div className="mt-4">
            <button onClick={() => handleConfirmation(true)} className="bg-green-500 text-white px-4 py-2 rounded mr-2">
              Дрон обнаружен
            </button>
            <button onClick={() => handleConfirmation(false)} className="bg-red-500 text-white px-4 py-2 rounded">
              Нет
            </button>
          </div>
        </div>
